...
export const ChatSideBar: React.FC<ChatProps> = ({
    ...
    useDetectClickOutside(dropdownRef as React.RefObject<HTMLDivElement>, () => {
        setOpenMenuId(null);
    });
        ...
        <aside className="flex flex-col w-64 h-screen px-4 py-8 overflow-y-auto bg-white">
            ...
            <div className="flex flex-col justify-between flex-1 mt-6">
                <nav>
                    {filteredPdfs.map(pdf => (
                        ...
                        >
                            ...
                            {openMenuId === pdf.id && (
                                <div ref={dropdownRef} className="absolute right-4 top-full mt-2 w-40 bg-white border border-gray-200 rounded shadow-lg z-20">
                ...
                {filteredPdfs.length === 0 && (
                    <div className="text-gray-500 text-center mt-4" onClick={() => navigate("/upload")}>
                        No PDFs found
                    </div>
                )}
                ...
                <a className="flex items-center px-4 -mx-2">
                    <img
                        ...
                        alt="avatar"
                    />
                    <span className="mx-2 font-medium text-gray-800">{name}</span>
                </a>
            </div>
        </aside>
    )
