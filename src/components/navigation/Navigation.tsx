...
export const Navigation = () => {
  ...
  const base = "block px-4 py-2 rounded text-white transition-colors duration-200";
  ...
  const Items = () => (
    <>
      <NavLink to="/" end className={({ isActive }) => `${base} ${linkBg(isActive)}`}>
        Home
      </NavLink>

      {isAuthenticated && (
        <NavLink to="/upload" className={({ isActive }) => `${base} ${linkBg(isActive)}`}>
          Upload
        </NavLink>
      )}

      {isAuthenticated ? (
        <button onClick={() => logout()} className={`${base} ${logoutBg}`}>
          Log Out
        </button>
      ) : (
        <button
    ...
    <nav className="bg-gray-800">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          <div className="text-white font-bold text-xl">Paper Pilot</div>
